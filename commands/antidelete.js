...
function loadAntideleteConfig(sessionId) {
    return readSessionData(sessionId, 'antidelete.json', { enabled: false });
}

// Save config
function saveAntideleteConfig(sessionId, config) {
    writeSessionData(sessionId, 'antidelete.json', config);
}
...
async function handleAntideleteCommand(sock, chatId, message, match) {
    ...
    const config = loadAntideleteConfig(sessionId);
    ...
    saveAntideleteConfig(sessionId, config);
...
async function storeMessage(sock, message) {
    try {
        ...
        const messageStore = getSessionCache(sessionId, 'messageStore');
...
async function handleMessageRevocation(sock, revocationMessage) {
    try {
        ...
        // Target the owner's specific "You" chat
