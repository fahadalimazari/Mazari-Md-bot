...
function initConfig(sessionId) {
    return readSessionData(sessionId, 'autotyping.json', { enabled: false });
}
...
async function autotypingCommand(sock, chatId, message, args = []) {
    try {
        ...
        writeSessionData(sessionId, 'autotyping.json', config);
...
function isAutotypingEnabled(sessionId) {
    try {
        const config = initConfig(sessionId);
...
async function handleAutotypingForMessage(sock, chatId, userMessage) {
    if (isAutotypingEnabled(getSessionId(sock))) {
        try {
