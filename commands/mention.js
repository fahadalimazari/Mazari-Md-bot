...
function loadState(sessionId) {
	try {
        const state = readSessionData(sessionId, 'mention.json', { enabled: false, assetPath: '', type: 'text' });
...
function saveState(sessionId, state) {
	writeSessionData(sessionId, 'mention.json', state);
}
...
async function setMentionCommand(sock, chatId, message, isOwner) {
    ...
    const stateBefore = loadState(sessionId);
    try {
