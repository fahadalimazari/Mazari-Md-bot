...
exports.convertMonospace = (text) => {
    const normal = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    const monospace = "𝙰𝙱𝙲𝙳𝙴𝙵𝙶𝙷𝙸𝙹𝙺𝙻𝙼𝙽𝙾𝙿𝚀𝚁𝚂𝚃𝚄𝚅𝚆𝚇𝚈𝚉𝚊𝚋𝚌𝚍𝚎𝚏𝚐𝚑𝚒𝚓𝚔𝚕𝚖𝚗𝚘𝚙𝚚𝚛𝚜𝚝𝚞𝚟𝚠𝚡𝚢𝚣𝟶𝟷𝟸𝟹𝟺𝟻𝟼𝟽𝟾𝟿";
    let styled = "";
    for (let char of text) {
        const index = normal.indexOf(char);
        styled += index > -1 ? monospace.slice(index * 2, (index + 1) * 2) : char;
    }
    return styled;
};

exports.reSize = async (buffer, options = {}) => {
    try {
        if (!buffer || typeof buffer !== 'object') {
            console.log("⚠️ Invalid buffer passed to reSize");
            return null;
        }

        // Automatically handle Git LFS pointer files (common in cloud/hosted checkouts)
        if (Buffer.isBuffer(buffer) && buffer.toString('utf-8', 0, 50).startsWith('version https://git-lfs.github.com')) {
        ...
        return await new Promise((resolve) => {
            img.resize({ w: width, h: height }).getBuffer(Jimp.MIME_JPEG || 'image/jpeg', (err, buff) => {
...
exports.getGroupMetadata = async (sock, jid) => {
    const sessionId = sock?.user?.id ? sock.user.id.split(':')[0].split('@')[0] : 'default';
    ...
    if (groupMetadataPromises.has(cacheKey)) {
        return groupMetadataPromises.get(cacheKey);
    }
    ...
    const promise = (async () => {
        try {
            ...
            // Cache for 5 minutes (reduced pressure on network)
            setTimeout(() => groupCache.delete(cacheKey), 5 * 60 * 1000);
            return metadata;
        } catch (error) {
            // Ignore missing group errors silently to prevent log spam
            if (!error.message.includes('not-authorized')) {
                console.error(`Error fetching group metadata for ${jid}:`, error.message);
            }
            throw error;
        } finally {
            groupMetadataPromises.delete(cacheKey);
        }
    })();

    groupMetadataPromises.set(cacheKey, promise);
    return promise;
};
