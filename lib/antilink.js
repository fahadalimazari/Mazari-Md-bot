...
const { getAntilink, isSudo } = require('../lib/index');
const { incrementTempWarning, resetTempWarning } = require('./tempWarnings');
...
async function Antilink(msg, sock) {
    ...
    try {
        ...
        if (action === 'kick') {
            ...
            resetTempWarning(sessionId, chatId, sender, 'antilink');
            return;
        }

        if (action === 'warn') {
            const warningCount = incrementTempWarning(sessionId, chatId, sender, 'antilink');
