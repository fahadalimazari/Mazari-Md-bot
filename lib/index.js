...
const { incrementTempWarning, resetTempWarning, getTempWarning, clearSessionTempWarnings } = require('./tempWarnings');
...
async function setAntiStatus(groupId, enabled, action = 'warn') {
    try {
        const data = loadUserGroupData();
...
async function getAntiStatus(groupId) {
    try {
        ...
        const entry = data.antistatus[groupId];
        if (typeof entry === 'boolean') {
            return { enabled: entry, action: 'warn' };
        }
        return {
