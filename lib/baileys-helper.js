...
async function backupSession(phoneNumber) {
  // 1. Concurrency control: only one backup runs at a time per session
  if (activeBackups.has(phoneNumber)) {
    pendingBackups.set(phoneNumber, true);
    return false;
  }
  ...
  const backupPromise = (async () => {
    ...
    try {
      let lastBackupTime = global.sessionLastBackupTime?.get(phoneNumber) || 0;
      const currentBackupTime = Date.now();
      const backupData = {};
      let filesRead = 0;
      ...
      for (const file of files) {
        if (file.endsWith('.json')) {
          ...
          // 2. Memory efficiency: read only files modified since last backup, or creds.json
          if (stat.mtimeMs >= lastBackupTime || file === 'creds.json') {
      ...
      if (filesRead === 0 && lastBackupTime > 0) {
        // No files modified, no need to lock DB or send anything
        return true;
      }
      ...
      await safeUpdateSessionData(phoneNumber, (existing) => {
        ...
        // 3. Restore compatibility: keep existing unchanged files from DB to avoid re-reading and re-allocating
        for (const file of files) {
          if (file.endsWith('.json')) {
            if (backupData[file] !== undefined) {
              newBackup[file] = backupData[file];
            } else if (existingBackup[file] !== undefined) {
              newBackup[file] = existingBackup[file];
            }
          }
        }

        return { ...existing, backup: newBackup };
      ...
      const hasCreds = backupData['creds.json'] || (fileCount > 0 && fs.existsSync(path.join(sessionPath, 'creds.json'))) ? '✓' : '✗';
      console.log(chalk.gray(`[BACKUP FILES] ${phoneNumber}: ${fileCount} files (${filesRead} updated) | creds.json ${hasCreds}`));

      let isRegistered = false;
      try {
        const credsContent = backupData['creds.json'] || fs.readFileSync(path.join(sessionPath, 'creds.json'), 'utf8');
        if (credsContent) {
      ...
      if (updateErr) {
        ...
        if (!global.sessionLastBackupTime) global.sessionLastBackupTime = new Map();
        global.sessionLastBackupTime.set(phoneNumber, currentBackupTime);
      }

      console.log(chalk.gray(`[BACKUP VERIFY] phone: ${phoneNumber}`));
      console.log(chalk.gray(`[BACKUP VERIFY] backup exists: ${!updateErr}`));
      console.log(chalk.gray(`[BACKUP VERIFY] file count: ${fileCount}`));

      backupExists = !updateErr;
      return backupExists;
...
supabaseSaved=false`));
  ...
  activeBackups.set(phoneNumber, backupPromise);

  try {
    return await backupPromise;
  } finally {
    activeBackups.delete(phoneNumber);
    if (pendingBackups.has(phoneNumber)) {
      pendingBackups.delete(phoneNumber);
      // Run the queued backup safely to capture any events missed during processing
      setTimeout(() => backupSession(phoneNumber), 500);
    }
  }
}
...
// Global 5-Minute Safe Memory & Temp Data Cleaner
setInterval(() => {
  // Prevent GC during massive active backup operations to avoid blocking
  if (activeBackups.size > 0) return;

  const memBefore = process.memoryUsage();
  console.log(`\n🧹 [MEMORY CLEANER] Before:`);
  ...
  console.log(`🧹 ArrayBuffers: ${(memBefore.arrayBuffers / 1024 / 1024).toFixed(2)} MB`);

  let removedCount = 0;
  const now = Date.now();

  // 1. Clean processedMessageIds older than 5 minutes
  ...
  // Bound limit for safety: if still too large, delete oldest
  if (processedMessageIds.size > 2000) {
    const overflow = processedMessageIds.size - 2000;
    const keysToDelete = Array.from(processedMessageIds.keys()).slice(0, overflow);
    keysToDelete.forEach(k => { processedMessageIds.delete(k); removedCount++; });
  }

  // 2. Clean processedReactions Set
  if (processedReactions.size > 2000) {
    const overflow = processedReactions.size - 2000;
    const keysToDelete = Array.from(processedReactions).slice(0, overflow);
    keysToDelete.forEach(k => { processedReactions.delete(k); removedCount++; });
  }

  // 3. Clean pairingTimeouts and waiters safely
  for (const [phone, timeout] of pairingTimeouts.entries()) {
    if (sessionStates.get(phone) === 'CONNECTED') {
      clearTimeout(timeout);
      pairingTimeouts.delete(phone);
      removedCount++;
    }
  }
  for (const [phone, attemptId] of pairingAttemptIds.entries()) {
    if (sessionStates.get(phone) === 'CONNECTED') {
      pairingAttemptIds.delete(phone);
      removedCount++;
    }
  }

  // 4. Clean reconnect counters safely (not a blind clear)
  for (const [phone, count] of reconnectCounters.entries()) {
    if (sessionStates.get(phone) === 'CONNECTED' || !sessions.has(phone)) {
      reconnectCounters.delete(phone);
      removedCount++;
    }
  }

  // 5. Clean stale sessionUpdateLocks
  for (const [phone, lock] of sessionUpdateLocks.entries()) {
    if (!sessions.has(phone)) {
      sessionUpdateLocks.delete(phone);
      removedCount++;
    }
  }

  console.log(`🧹 [MESSAGE CLEANER] Cleaning temporary message data...`);
  console.log(`🧹 [MESSAGE CLEANER] Removed: ${removedCount} temporary entries`);
  ...
  console.log(`📊 [MEMORY CLEANER] After:`);
...
async function initSession(phoneNumber, options = {}) {
  ...
  // skipRestore: set by requestPairingCode when a fresh pairing is needed — do NOT restore old auth
...
isOwnedByCurrentServer=${data?.claimed ? 'true' : 'false'}`));
  ...
  try {
    ...
    if (sharedDbData && sharedDbData.session_data) {
      const owner = sharedDbData.session_data.owner_id;
      const lastActive = sharedDbData.session_data.last_active || 0;
  ...
  // skipRestore is set for manual re-pair: clean local auth is already prepared by requestPairingCode
  if (skipRestore) {
    console.log(chalk.yellow(`[REPAIR] Skipping Supabase restore — fresh pairing auth already prepared for ${phoneNumber}`));
    usePairingCode = true;
  } else
  try {
    let dbData = sharedDbData;
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!isComplete) {
        ...
        if (!isRegistered) {
          ...
          usePairingCode = true;
          if (wasManualPair && isRegistered) {
            console.warn(chalk.yellow(`[RESTORE] Manual pairing with registered backup — fresh auth should have been used. Forcing clean.`));
            try { fs.unlinkSync(path.join(sessionPath, 'creds.json')); } catch (e) { /* ignore */ }
            usePairingCode = true;
          }
        }
      }
...
if (usePairingCode && !state.creds.registered) {
  ...
  if (isPendingPairing) {
    ...
    }, 300); // Drastically reduced latency from 3000ms to 300ms to deliver code instantly
  }
}
...
sock.ev.on('connection.update', async (update) => {
  ...
  if (connection === 'close') {
    ...
    if (shouldReconnect) {
      ...
      // 401 error handling – treat differently if a pairing is in progress
      if (statusCode === 401 && !isGenuineLogout) {
        if (isPairing) {
          // During pairing, a 401 typically means the server rejected the temporary auth.
          // Instead of aborting, log and allow the reconnection logic below to retry.
          console.log(chalk.yellow(`⚠️ [PAIRING 401] ${phoneNumber}: 401 received during pairing. Will retry connection after backoff.`));
        } else if (activeLifecycle && !activeLifecycle.hasEverOpened) {
          await updateSessionStatus(phoneNumber, 'NEEDS_PAIRING');
          sessionStates.set(phoneNumber, 'NEEDS_PAIRING');
          if (activeLifecycle) activeLifecycle.state = 'NEEDS_PAIRING';
          // Stop the current session, clean local auth, then initiate a fresh pairing flow
          await stopSession(phoneNumber);
          // Remove local auth files so Baileys treats this as a fresh session
          cleanupSessionFolder(phoneNumber);
          // Initiate a new pairing code request and resolve any waiting callers
          try {
            const code = await sock.requestPairingCode(phoneNumber);
            const formatted = code?.match(/.{1,4}/g)?.join('-') || code;
            console.log(chalk.black(chalk.bgGreen(` [CODE] ${phoneNumber}: `)), chalk.bold.white(formatted));
            pairingCodesStore.set(phoneNumber, formatted);
            // Resolve any pending waiters with the new code
            const waiters = pairingWaiters.get(phoneNumber) || [];
            waiters.forEach(w => w.resolve({ success: true, code: formatted }));
            pairingWaiters.delete(phoneNumber);
          } catch (err) {
            console.log(chalk.yellow(`⚠️ Pairing failed for ${phoneNumber} after 401 restore: ${err.message}`));
            pairingCodesStore.set(phoneNumber, 'ERROR: ' + (err.message || 'Failed to generate code'));
            const waiters = pairingWaiters.get(phoneNumber) || [];
            waiters.forEach(w => w.reject(new Error(err.message || 'Failed to generate code')));
            pairingWaiters.delete(phoneNumber);
          }
          return; // Abort further reconnect attempts
        }
      ...
      // Handle disconnects during pairing differently
      if (isPairing) {
        if (statusCode === 515) {
          console.log(chalk.yellow(`🔄 [DISCONNECT] ${phoneNumber}: 515 Stream Errored during pairing. Reconnecting to preserve session...`));
          delay = 5000;
        } else {
          console.log(chalk.yellow(`🔄 [PAIRING DISCONNECT] ${phoneNumber}: Received ${statusCode} during pairing. Will attempt reconnect without aborting.`));
          // Do not stop session; keep socket alive for retry
        }
      } else {
        // Non‑pairing flow: safe to stop the session before reconnect
        await stopSession(phoneNumber);
      }
      ...
      try {
        ...
        if (!supabase.isMock) {
          ...
          if (!vData && !vErr) {
            ...
            if (rpcData) {
              const row = rpcData.find(r => String(r.phone_number).split(':')[0].split('@')[0].replace(/[^0-9]/g, '') === normReq);
              if (row) vData = { phone_number: row.phone_number, session_data: row.session_data };
            }
          }
...
async function requestPairingCode(phoneNumber, requesterIsOwner = false) {
  ...
  // ── MANUAL RE-PAIR LOGIC ────────────────────────────────────────────────────
  // If the phone already has a registered Supabase backup, we must NOT restore
  // those old creds into the active auth directory — that causes 401 + failure.
  // Instead: wipe the LOCAL session folder (keep Supabase backup intact), then
  // create a completely fresh Baileys auth state for the new pairing socket.
  let skipRestoreForPair = false;
  try {
    const sessionBaseDir = process.env.SESSION_DIR
      ? path.resolve(process.env.SESSION_DIR)
      : path.join(__dirname, '../session');
    const localSessionPath = path.join(sessionBaseDir, phoneNumber);
    const normNum = String(phoneNumber).split(':')[0].split('@')[0].replace(/[^0-9]/g, '');

    // Fetch the DB row to check if a registered backup exists
    let dbRow = null;
    let { data: d1 } = await supabase.from('bot_sessions').select('phone_number, session_data').eq('phone_number', normNum).maybeSingle();
    if (!d1) {
      const { data: d2 } = await supabase.from('bot_sessions').select('phone_number, session_data').eq('phone_number', '+' + normNum).maybeSingle();
      d1 = d2;
    }
    if (!d1) {
      const { data: rpcData } = await supabase.rpc('get_all_sessions');
      if (rpcData) {
        const row = rpcData.find(r => String(r.phone_number).split(':')[0].split('@')[0].replace(/[^0-9]/g, '') === normNum);
        if (row) d1 = { phone_number: row.phone_number, session_data: row.session_data };
      }
    }
    dbRow = d1;

    const backup = dbRow?.session_data?.backup;
    if (backup && backup['creds.json']) {
      let backupIsRegistered = false;
      try {
        const parsed = typeof backup['creds.json'] === 'string'
          ? JSON.parse(backup['creds.json'])
          : backup['creds.json'];
        backupIsRegistered = parsed.registered === true;
      } catch (e) { }

      if (backupIsRegistered) {
        console.log(chalk.yellow(`[REPAIR] Existing registered session found for ${phoneNumber} — starting fresh manual pairing`));
        // Wipe local session folder so Baileys starts with a blank auth state
        ...
        console.log(chalk.yellow(`[REPAIR] Old session ignored/stopped — clean pairing auth created`));
        skipRestoreForPair = true;
      }
    }
  } catch (repairErr) {
    console.warn(chalk.yellow(`[REPAIR] Could not check DB for existing backup: ${repairErr.message}`));
  }
  // ── END MANUAL RE-PAIR LOGIC ────────────────────────────────────────────────
  ...
  initSession(phoneNumber, { usePairingCode: true, force: true, isOwner: requesterIsOwner, skipRestore: skipRestoreForPair }).catch(err => {
