  ...
  const next = lock.then(async () => {
    try {
      ...
      if (!data) {
          ...
          if (claimErr) console.error(chalk.red(`[SESSION PERSIST ERROR] ${phoneNumber} claim_ownership fallback failed: ${claimErr.message}`));
          
          const { error: rpcErr } = await supabase.rpc('update_session_data', { p_phone_number: phoneNumber, p_session_data: newData });
          if (rpcErr) console.error(chalk.red(`[SESSION PERSIST ERROR] ${phoneNumber} RPC failed after claim: ${rpcErr.message}`));
      } else {
          const { error: rpcErr } = await supabase.rpc('update_session_data', { p_phone_number: phoneNumber, p_session_data: newData });
