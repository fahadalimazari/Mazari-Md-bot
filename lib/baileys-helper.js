...
if (!force && (currentState === 'CONNECTING' || currentState === 'RECONNECTING' || currentState === 'BACKING_OFF' || currentState === 'CONNECTED' || currentState === 'OPEN')) {
...
isStale=N/A`));
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
       ...
       if (activeLifecycle) activeLifecycle.state = 'OPEN';
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        activeLifecycle.reconnectTimer = setTimeout(() => initSession(phoneNumber, { usePairingCode: usePairingCode && sessionStates.get(phoneNumber) !== 'CONNECTED', isOwner: isOwner }), delay);
      } else {
