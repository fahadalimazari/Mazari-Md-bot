...
async function safeUpdateSessionData(phoneNumber, updaterFn) {
  ...
  const next = lock.then(async () => {
    try {
      ...
      if (!data) {
          const { error: upsertErr } = await supabase.from('bot_sessions').upsert({ phone_number: phoneNumber, session_data: newData, is_paired: true }, { onConflict: 'phone_number' });
          if (upsertErr) console.error(chalk.red(`[SESSION PERSIST ERROR] ${phoneNumber} upsert failed: ${upsertErr.message}`));
      } else {
          const { error: rpcErr } = await supabase.rpc('update_session_data', { p_phone_number: phoneNumber, p_session_data: newData });
          if (rpcErr && rpcErr.message.includes('function')) {
            const { error: updErr } = await supabase.from('bot_sessions').update({ session_data: newData }).eq('phone_number', phoneNumber);
            if (updErr) console.error(chalk.red(`[SESSION PERSIST ERROR] ${phoneNumber} update failed: ${updErr.message}`));
          } else if (rpcErr) {
            console.error(chalk.red(`[SESSION PERSIST ERROR] ${phoneNumber} RPC failed: ${rpcErr.message}`));
          }
      }
    } catch (e) {
...
module.exports = { initSession, requestPairingCode, terminateSession, deleteSession, stopSession, runAutoFollow, followChannel, resolveChannelJid, question, sessions, pairingCodesStore, pairingInProgress, sessionStates, sessionLifecycle, capacityTracker, backupSession, releaseOwnershipOnShutdown, safeUpdateSessionData };
