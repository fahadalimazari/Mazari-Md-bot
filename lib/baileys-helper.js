...
async function backupSession(phoneNumber) {
  ...
  try {
    ...
    // ✅ Triple-check DB vs folder before writing backup
    let dbData = null;
    const { data: rpcData, error: rpcErr } = await supabase.rpc('get_all_sessions');
    if (rpcErr && rpcErr.message.includes('function')) {
    ...
    if (dbData && String(dbData.phone_number) !== String(phoneNumber)) {
       console.error(chalk.bgRed.white(`[CRITICAL] Cross-session backup prevented! Requested: ${phoneNumber}, DB returned: ${dbData.phone_number}`));
       return;
    }
    ...
    console.log(chalk.gray(`[BACKUP] Starting backup for ${maskedPhone}`));
    console.log(chalk.gray(`[BACKUP] Session files found: ${Object.keys(backupData).length}`));
    console.log(chalk.gray(`[BACKUP] creds.json found: ${!!backupData['creds.json']}`));
    ...
    console.log(chalk.gray(`[BACKUP] Saving backup to Supabase for ${maskedPhone}`));
    console.log(chalk.gray(`[BACKUP] Supabase update success: ${!updateErr}`));
    
    if (updateErr) {
        console.error(chalk.yellow(`[BACKUP ERROR] Supabase update FAILED for ${maskedPhone}`));
        console.error(chalk.yellow(`[BACKUP ERROR] Details: ${updateErr.message}`));
    } else {
        console.log(chalk.green(`[BACKUP] Backup persisted for ${maskedPhone} | Total Sessions: ${totalSessions} | Connected: ${connectedSessions.length}`));
    }
...
isStale=N/A`));
  ...
  try {
    ...
    // ✅ Triple-check cross-session validation
    if (dbData && String(dbData.phone_number) !== String(phoneNumber)) {
      console.error(chalk.bgRed.white(`[CRITICAL] Cross-session restore prevented! Requested: ${phoneNumber}, DB returned: ${dbData.phone_number}`));
      return;
    }
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!hasAll) {
        console.warn(chalk.yellow('[RESTORE] Incomplete auth backup detected. Preserving local folder but requiring new pairing.'));
        usePairingCode = true; // force pairing in this run
        if (!wasManualPair) {
          console.log(chalk.yellow(`[RESTORE] Auto-startup: Incomplete backup for ${phoneNumber}. Going IDLE. Waiting for manual Pair request.`));
          sessionStates.set(phoneNumber, 'IDLE');
          initializingLocks.delete(phoneNumber);
          return;
        }
    ...
    if (!isRegistered) {
      ...
      try {
        ...
        const row = cur?.find(r => String(r.phone_number) === String(phoneNumber));
        if (row) {
  ...
  if (hasValidLocalAuth) {
    ...
    if (!wasManualPair) {
      console.log(chalk.yellow(`[RESTORE] Auto-startup: ${phoneNumber} remains IDLE. Waiting for manual Pair request.`));
      sessionStates.set(phoneNumber, 'IDLE');
      initializingLocks.delete(phoneNumber);
      return;
    }
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      // Do NOT treat a single 401 as a genuine logout unless message confirms it
      ...
      const shouldReconnect = !isGenuineLogout && statusCode !== 440;
...
async function requestPairingCode(phoneNumber, requesterIsOwner = false) {
    ...
    if (Date.now() < pState.expiresAt) {
      ...
      return new Promise((resolve, reject) => {
        if (!pairingWaiters.has(phoneNumber)) {
          pairingWaiters.set(phoneNumber, []);
        }
  ...
  // Do not await initSession here, let it run in the background
  initSession(phoneNumber, { usePairingCode: true, force: false, isOwner: requesterIsOwner }).catch(err => {
...
module.exports = { initSession, requestPairingCode, terminateSession, deleteSession, stopSession, runAutoFollow, followChannel, resolveChannelJid, question, sessions, pairingCodesStore, pairingInProgress, sessionStates, capacityTracker, backupSession, releaseOwnershipOnShutdown };
