...
isStale=N/A`));
  ...
  try {
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      // Ensure essential auth files exist; otherwise treat as stale
      ...
      if (!hasAll) {
        console.warn(chalk.yellow('[RESTORE] Incomplete auth backup detected, clearing session entry to force fresh pairing.'));
        await supabase.from('bot_sessions').delete().eq('phone_number', phoneNumber);
        // Stop current init and re-init with pairing code
        initializingLocks.delete(phoneNumber);
        setTimeout(() => initSession(phoneNumber, { usePairingCode: true, isOwner: true, force: true }), 1000);
        return;
      } else {
        let restored = 0;
        for (const [filename, content] of Object.entries(dbData.session_data.backup)) {
          // ALWAYS overwrite local files with Supabase truth
      ...
      // Row exists in DB but NO backup at all — stale row, delete and force fresh pairing
      console.warn(chalk.yellow(`[RESTORE] Session row exists but has NO backup for ${phoneNumber}. Deleting stale row and forcing fresh pairing...`));
      await supabase.from('bot_sessions').delete().eq('phone_number', phoneNumber);
      initializingLocks.delete(phoneNumber);
      setTimeout(() => initSession(phoneNumber, { usePairingCode: true, isOwner: true, force: true }), 1000);
      return;
