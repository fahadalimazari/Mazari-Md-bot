...
async function backupSession(phoneNumber) {
  ...
  try {
    ...
    const { error: updateErr } = await supabase.from('bot_sessions').update({ session_data: newData }).eq('phone_number', phoneNumber);
...
isStale=N/A`));
  ...
  try {
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (restored > 0) {
          console.log(chalk.green(`[RESTORE] Restored ${restored} session files from Supabase for ${phoneNumber}`));
          console.log(chalk.gray(`[RESTORE]
phone=${phoneNumber}
...
restoreAttempted=true`));
        ...
        console.log(chalk.gray(`[RESTORE]
phone=${phoneNumber}
foundInSupabase=${!!dbData}
authStateAvailable=false
...
restoreAttempted=false`));
  ...
  console.log(chalk.gray(`[SESSION REGISTER]
phone=${phoneNumber}
  ...
  sock.ev.on('creds.update', async () => {
    try {
      ...
      global.sessionBackupTimeouts.set(phoneNumber, setTimeout(() => {
        // ALWAYS backup credentials when they update, even if just CONNECTING
    ...
    if (connection === 'close') {
      ...
      if (shouldReconnect) {
        ...
        if (errorMessage.includes('Bad MAC') || errorMessage.includes('Ciphertext') || attempts > 5) {
          ...
          if (attempts > 8) {
            console.log(chalk.bgRed(`💥 [FATAL] ${phoneNumber}: Persistent encryption or authentication error (401). Halting session WITHOUT deleting data...`));
            sessionStates.set(phoneNumber, 'IDLE');
            // Do NOT delete the session from Supabase or disk. 
            // The user explicitly requested to never delete sessions automatically on deploy/restart.
            return;
          }
        }
        ...
        // Fix 515/Network disconnect logic
...
socketRecreated=true`));
      ...
      try {
        const { data: exist, error: existErr } = await supabase.from('bot_sessions').select('phone_number').eq('phone_number', phoneNumber).maybeSingle();
        if (existErr) console.error(chalk.red(`[DB ERROR] Failed to check session for ${phoneNumber}: ${existErr.message}`));
        
        if (exist) {
          const { error: updErr } = await supabase.from('bot_sessions').update({ is_paired: true }).eq('phone_number', phoneNumber);
          if (updErr) console.error(chalk.red(`[DB ERROR] Failed to update is_paired for ${phoneNumber}: ${updErr.message}`));
        } else {
          const { error: insErr } = await supabase.from('bot_sessions').insert({ phone_number: phoneNumber, is_paired: true });
          if (insErr) console.error(chalk.red(`[DB ERROR] Failed to insert session for ${phoneNumber}: ${insErr.message}`));
        }
      } catch (dbErr) {
