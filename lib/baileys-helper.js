...
async function safeUpdateSessionData(phoneNumber, updaterFn) {
  ...
  const next = lock.then(async () => {
    try {
      ...
      if (!data) {
          await supabase.from('bot_sessions').upsert({ phone_number: phoneNumber, session_data: newData, is_paired: true }, { onConflict: 'phone_number' });
      } else {
      ...
      console.error(chalk.yellow(`[DB UPDATE ERROR] ${phoneNumber}: ${e.message}`));
    }
  }).catch(() => {});
    ...
    if (connection === 'close') {
      ...
      setTimeout(async () => {
          try {
              ...
              console.log(chalk.gray(`[SESSION PERSIST] ${phoneNumber} backup saved`));
              
              await updateSessionStatus(phoneNumber, 'ACTIVE');
              console.log(chalk.gray(`[SESSION PERSIST] ${phoneNumber} status ACTIVE saved`));
          } catch (err) {
