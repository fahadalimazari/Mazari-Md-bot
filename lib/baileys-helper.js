...
async function backupSession(phoneNumber) {
  ...
  try {
    ...
    // Safety check: Prevent lagging backups from resurrecting an INACTIVE session
    if (newData.status === 'INACTIVE' || newData.status === 'NEEDS_PAIRING' || newData.status === 'CONFLICT') {
    ...
    const backupFilenames = Object.keys(backupData).join(', ');
    console.log(chalk.gray(`[BACKUP FILES] ${phoneNumber}: ${backupFilenames}`));
...
async function updateSessionStatus(phoneNumber, status) {
  try {
      ...
      if (data) {
          ...
          if (sData.status === status) return;
          sData.status = status;
          
          const { error: rpcErr } = await supabase.rpc('update_session_data', { p_phone_number: phoneNumber, p_session_data: sData });
          if (rpcErr && rpcErr.message.includes('function')) {
              await supabase.from('bot_sessions').update({ session_data: sData }).eq('phone_number', phoneNumber);
          }
      }
  } catch (err) {
      console.error(chalk.yellow(`[SESSION STATUS] Error updating status for ${phoneNumber}: ${err.message}`));
  }
}
  ...
  try {
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!isComplete) {
        ...
        if (!wasManualPair) {
           console.warn(chalk.yellow(`[RESTORE] Preserving backup and waiting for manual pairing`));
        ...
        const hb = setInterval(async () => {
          try {
            ...
            if (!fetchErr && existing) {
              const sData = existing.session_data || {};
              // Prevent lagging heartbeat from reverting a status change
              if (sData.status === 'INACTIVE' || sData.status === 'NEEDS_PAIRING' || sData.status === 'CONFLICT') {
                  clearInterval(hb);
                  return;
              }
              const merged = { ...sData, last_active: Date.now() };
              await supabase
                .from('bot_sessions')
...
module.exports = { initSession, requestPairingCode, terminateSession, deleteSession, stopSession, runAutoFollow, followChannel, resolveChannelJid, question, sessions, pairingCodesStore, pairingInProgress, sessionStates, sessionLifecycle, capacityTracker, backupSession, releaseOwnershipOnShutdown };
