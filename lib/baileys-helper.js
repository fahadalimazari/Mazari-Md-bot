...
if (!supabase.isMock && !force) {
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      try {
        const { data: exist } = await supabase.from('bot_sessions').select('phone_number').eq('phone_number', phoneNumber).maybeSingle();
        if (exist) {
          await supabase.from('bot_sessions').update({ is_paired: true }).eq('phone_number', phoneNumber);
        } else {
          await supabase.from('bot_sessions').insert({ phone_number: phoneNumber, is_paired: true });
        }
      } catch (dbErr) {
