    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!hasAll) {
        ...
        if (restored > 0) {
          ...
          if (!isRegistered) {
            console.warn(chalk.yellow(`[RESTORE] Backup creds for ${phoneNumber} are UNREGISTERED (incomplete pairing). Clearing stale backup to force fresh pairing...`));
            cleanupSessionFolder(phoneNumber);
            // Wipe invalid backup from Supabase so next restart doesn't repeat this
            try {
              const { data: cur } = await supabase.from('bot_sessions').select('session_data').eq('phone_number', phoneNumber).maybeSingle();
              if (cur) {
                const cleaned = { ...(cur.session_data || {}) };
                delete cleaned.backup;
                await supabase.from('bot_sessions').update({ session_data: cleaned }).eq('phone_number', phoneNumber);
              }
      ...
      // Row exists but NO backup at all — clean local folder and do fresh pairing in this run
      console.warn(chalk.yellow(`[RESTORE] No backup found for ${phoneNumber}. Cleaning up for fresh pairing (no recursive reinit).`));
      cleanupSessionFolder(phoneNumber);
      usePairingCode = true; // force pairing in this run — NO recursive initSession call
    }
  } catch (e) {
