...
const { clearSessionTempWarnings } = require('./tempWarnings');
  ...
  try {
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!isComplete) {
        ...
        // [PAIR RECOVERY] After restore, handle manual pairing with registered creds
        if (wasManualPair && isRegistered) {
          console.warn(chalk.yellow(`[PAIR RECOVERY] Manual pairing with registered backup detected. Skipping restored auth and forcing fresh pairing.`));
        ...
        if (!isRegistered) {
          ...
          // If this is a manual pairing request and the restored credentials are already registered,
          // we must ignore them and force a fresh pairing flow. The remote Supabase backup stays untouched.
          if (wasManualPair && isRegistered) {
            console.warn(chalk.yellow(`[RESTORE] Manual pairing with registered backup detected. Skipping restored auth and forcing fresh pairing.`));
            // Remove locally restored creds.json to avoid using registered auth.
            try { fs.unlinkSync(path.join(sessionPath, 'creds.json')); } catch (e) { /* ignore if already removed */ }
            // Force use of fresh pairing code generation.
            usePairingCode = true;
          }
        }
      }
...
async function stopSession(phoneNumber) {
  ...
  clearSessionTempWarnings(phoneNumber);
  return true;
}
