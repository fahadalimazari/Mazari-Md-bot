...
isStale=N/A`));
  ...
  try {
    ...
    if (dbData && dbData.session_data && dbData.session_data.backup) {
      ...
      if (!hasAll) {
        console.warn(chalk.yellow('[RESTORE] Incomplete auth backup detected.'));
  ...
  if (hasValidLocalAuth) {
    ...
    console.log(chalk.yellow(`[RESTORE] No valid local auth found for ${phoneNumber}.`));
    if (!pairingInProgress.has(phoneNumber)) {
      cleanupSessionFolder(phoneNumber);
    }
    if (!wasManualPair) {
    ...
    if (isPendingPairing) {
      ...
      const timeout = setTimeout(async () => {
        if (sessionStates.get(phoneNumber) !== 'CONNECTED') {
          console.log(chalk.yellow(`⏳ [TIMEOUT] Pairing expired for ${phoneNumber}. Cleaning up...`));
